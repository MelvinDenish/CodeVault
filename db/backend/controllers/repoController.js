  ...
  try {
    await conn.execute(
      `UPDATE repositories 
       ...
       WHERE repo_id = :id AND owner_id = :ownerid`,
      {
        n: name || null, d: description || null, v: visibility || null,
        id: parseInt(req.params.id), ownerid: req.user.id
      }
    );
    await conn.commit();
...
async function deleteRepo(req, res) {
  ...
  try {
    const result = await conn.execute(
      `DELETE FROM repositories WHERE repo_id = :id AND owner_id = :ownerid`,
      { id: parseInt(req.params.id), ownerid: req.user.id }
    );
    await conn.commit();
...
async function toggleStar(req, res) {
  ...
  try {
    const existing = await conn.execute(
      `SELECT star_id FROM stars WHERE user_id = :ownerid AND repo_id = :rid`,
      { ownerid: req.user.id, rid: parseInt(req.params.id) }
    );

    if (existing.rows.length > 0) {
      await conn.execute(
        `DELETE FROM stars WHERE user_id = :ownerid AND repo_id = :rid`,
        { ownerid: req.user.id, rid: parseInt(req.params.id) }
      );
      await conn.commit();
      ...
      await conn.execute(
        `INSERT INTO stars (user_id, repo_id) VALUES (:ownerid, :rid)`,
        { ownerid: req.user.id, rid: parseInt(req.params.id) }
      );
      await conn.commit();

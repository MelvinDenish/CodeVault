...
async function addCollaborator(req, res) {
  ...
  try {
    ...
    const existing = await conn.execute(
      `SELECT collab_id FROM collaborators WHERE repo_id = :rid AND user_id = :collab_uid`,
      { rid: repoId, collab_uid: userId }
    );
    if (existing.rows.length > 0) {
      // Update role
      await conn.execute(
        `UPDATE collaborators SET role = :role WHERE repo_id = :rid AND user_id = :collab_uid`,
        { role: role || 'viewer', rid: repoId, collab_uid: userId }
      );
    } else {
      await conn.execute(
        `INSERT INTO collaborators (repo_id, user_id, role) VALUES (:rid, :collab_uid, :role)`,
        { rid: repoId, collab_uid: userId, role: role || 'viewer' }
      );
    }
    await conn.commit();
...
async function removeCollaborator(req, res) {
  ...
  try {
    await conn.execute(
      `DELETE FROM collaborators WHERE repo_id = :rid AND user_id = :collab_uid`,
      { rid: repoId, collab_uid: userId }
    );
    await conn.commit();
  ...
  try {
    ...
    const newRepo = await conn.execute(
      `INSERT INTO repositories (repo_name, description, owner_id, visibility)
       VALUES (:name, :desc, :fork_uid, 'public')
    ...
    await conn.execute(
      `INSERT INTO forks (original_repo, forked_repo, forked_by) VALUES (:orig, :fork, :fork_uid)`,
      { orig: originalRepoId, fork: forkedRepoId, fork_uid: userId }
    );
    ...
    await conn.execute(
      `INSERT INTO activity_logs (user_id, repo_id, action, details)
       VALUES (:fork_uid, :rid, 'fork', 'Forked from repo #' || :orig)`,
      { fork_uid: userId, rid: forkedRepoId, orig: originalRepoId }
    );
...
async function copyCommit(conn, commitId, newRepoId, newBranchId, userId) {
  ...
  const newCommit = await conn.execute(
    `INSERT INTO commits (repo_id, branch_id, author_id, message, commit_hash)
     VALUES (:rid, :bid, :author_uid, :msg, :hash)
     RETURNING commit_id INTO :cid`,
    {
      rid: newRepoId, bid: newBranchId, author_uid: userId,
